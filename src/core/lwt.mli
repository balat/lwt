...
module Private : sig
  ...
  module Sequence_associated_storage : sig
    ...
    (** Was [val current_storage : storage ref]. The [ref] VALUE cannot survive
        per-domain scheduler state: whoever read it would keep the cell of the
        domain that loaded the module, for good. [Lwt.Private] is alerted as
        internal, and its only consumers are in-tree ([Lwt_direct], [Lwt_main],
        [ppx_lwt]). *)
  end
