...
(** {3 Which domain these belong to}
    ...
    Every Lwt loop has its own notification pipe, and a notification id carries
    the identity of the pipe it belongs to. So {!make_notification} registers a
    notification on the CALLING domain, and its handler runs there, on the domain
    that created it, whichever thread or domain later sends it. That is what makes
    {!make_notification} with {!send_notification} the way to wake a particular
    Lwt loop from elsewhere; they, along with {!stop_notification} and
    {!set_notification}, are safe to call from any thread and any domain.
